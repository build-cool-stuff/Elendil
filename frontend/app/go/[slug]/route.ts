...
export async function GET(
  ...
  // 9. Fire Meta CAPI event (async, non-blocking)
  if (campaign.meta_pixel_id) {
    ...
    fireQRCodeScanEvent({
      ...
      encryptedToken: campaign.encrypted_access_token && campaign.encryption_iv
        ? {
  ...
  if (campaign.bridge_enabled && campaign.meta_pixel_id) {
    ...
    const response = NextResponse.redirect(bridgeUrl, { status: 302 })
  ...
  // Direct redirect without bridge - record scan here with Vercel geo
  // (Bridge page handles its own scan recording with BigDataCloud precision)
  recordScan({
